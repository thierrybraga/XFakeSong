...
def _write_json_atomic(path: Path, texto: str) -> None:
    """Escreve texto em `path` de forma atômica (tmp + os.replace)."""
...
def write_all(results: Dict[str, Any], output_dir: str) -> None:
    ...
    # ATÔMICO: `results.json` é o artefato que a retomada, a consolidação e a
    # promoção leem. Com `restart: on-failure:10` no compose, uma queda durante
    # a gravação deixava um JSON truncado no lugar do bom.
    _write_json_atomic(
        out / "results.json",
