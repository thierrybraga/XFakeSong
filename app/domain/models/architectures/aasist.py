...
# CAMADAS PRÓPRIAS DO AASIST (separadas de layers.py em 2026-08-20).
#
# Antes, AASIST e RawGAT-ST compartilhavam a mesma implementação de SincConv,
# GraphPool, GraphAttention, GraphReadout e dos blocos residuais. Isso já
# causou dano real: uma correção de fidelidade ao paper do RawGAT-ST mudou o
# AASIST junto, em silêncio — duas entradas oficiais do benchmark de uma vez.
# Cada arquitetura responde ao SEU paper, e os papers divergem; a implementação
# tem de poder divergir junto.
from app.domain.models.architectures.aasist_layers import (
    AasistGATConv,
    AasistGraphAttention,
    AasistGraphPool,
    AasistGraphReadout,
    AasistHSGAL,
    AasistHtrgGraphAttention,
    AasistMasterNodeSeed,
    AasistResidualBlock1D,
    AasistResidualBlock2D,
    AasistSincConv,
    AasistSpectralPositionEmbedding,
)

# Continuam COMPARTILHADOS de propósito:
#   - AMSoftmax*: infraestrutura de treino, usada também por trainer.py,
#     optimization.py e training_wizard.py — não é camada de arquitetura.
#   - AudioFeatureNormalization/AxisMaxAbs/Magnitude: plumbing de tensor sem
#     parâmetro de paper.
...
def _build_paper_aasist(
    ...
    spectral = AasistSpectralPositionEmbedding(name="aasist_spectral_position")(spectral)
    ...
    spectral = AasistGraphAttention(
        out_features=64, temperature=GAT_TEMPERATURE,
        dropout_rate=dropout_rate, name="aasist_gat_spectral",
    )(spectral)
    temporal = AasistGraphAttention(
        out_features=64, temperature=GAT_TEMPERATURE,
        dropout_rate=dropout_rate, name="aasist_gat_temporal",
    )(temporal)
    spectral = AasistGraphPool(0.5, name="aasist_pool_spectral")(spectral)
    temporal = AasistGraphPool(0.7, name="aasist_pool_temporal")(temporal)
    ...
    master1 = AasistMasterNodeSeed(64, name="aasist_master1")(spectral)
    master2 = AasistMasterNodeSeed(64, name="aasist_master2")(spectral)

    s1, t1, m1 = AasistHtrgGraphAttention(
        out_features=32, temperature=HSGAL_TEMPERATURE,
        dropout_rate=dropout_rate, name="aasist_hsgal_11",
    )([spectral, temporal, master1])
    s1 = AasistGraphPool(0.5, name="aasist_hpool_s1")(s1)
    t1 = AasistGraphPool(0.5, name="aasist_hpool_t1")(t1)
    s1_aug, t1_aug, m1_aug = AasistHtrgGraphAttention(
        out_features=32, temperature=HSGAL_TEMPERATURE,
    ...
    s2, t2, m2 = AasistHtrgGraphAttention(
        out_features=32, temperature=HSGAL_TEMPERATURE,
        dropout_rate=dropout_rate, name="aasist_hsgal_21",
    )([spectral, temporal, master2])
    s2 = AasistGraphPool(0.5, name="aasist_hpool_s2")(s2)
    t2 = AasistGraphPool(0.5, name="aasist_hpool_t2")(t2)
    s2_aug, t2_aug, m2_aug = AasistHtrgGraphAttention(
        out_features=32, temperature=HSGAL_TEMPERATURE,
...
def create_model(
    ...
    if architecture in LEGACY_VARIANTS:
        ...
        x = AasistResidualBlock1D(out_channels=128, kernel_size=3, name="res_block_1a")(x)
        x = AasistResidualBlock1D(out_channels=128, kernel_size=3, name="res_block_1b")(x)
        x = layers.MaxPooling1D(pool_size=3, name="res_pool_1")(x)

        # --- 3. Residual Blocks Group 2 ---
        x = AasistResidualBlock1D(out_channels=256, kernel_size=3, name="res_block_2a")(x)
        x = AasistResidualBlock1D(out_channels=256, kernel_size=3, name="res_block_2b")(x)
        ...
        x = AasistResidualBlock1D(out_channels=256, kernel_size=3, name="res_block_3a")(x)
        x = AasistResidualBlock1D(out_channels=256, kernel_size=3, name="res_block_3b")(x)
        ...
        # --- 8. Graph Pooling ---
        spectral_nodes = AasistGraphPool(
            ratio=0.5, name="graph_pool_spectral")(spectral_nodes)
        temporal_nodes = AasistGraphPool(
            ratio=0.5, name="graph_pool_temporal")(temporal_nodes)

        # --- 9. Graph Readout (max + attention) ---
        spec_readout = AasistGraphReadout(name="readout_spectral")(spectral_nodes)
        temp_readout = AasistGraphReadout(name="readout_temporal")(temporal_nodes)
