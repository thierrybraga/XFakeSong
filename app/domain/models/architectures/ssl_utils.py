...
def build_ssl_aasist_backend(x, dropout_rate: float = 0.3, proj_dim: int = 128,
    ...
    # Esta cabeca e uma pilha AASIST sobre embeddings SSL, entao consome as
    # camadas PROPRIAS do AASIST (separadas em 2026-08-20). Enquanto vinha de
    # layers.py, uma correcao de fidelidade no AASIST nao alcancava esta
    # ablacao — as duas divergiam em silencio.
    ...
    from app.domain.models.architectures.layers import TimeResizeLayer
    ...
    spectral, temporal, _master = AasistHtrgGraphAttention(
        out_features=32, temperature=100.0, dropout_rate=dropout_rate,
        name=f"{name}_hsgal")([spectral, temporal])

    spectral = AasistGraphPool(ratio=0.5, name=f"{name}_pool_spec")(spectral)
    temporal = AasistGraphPool(ratio=0.5, name=f"{name}_pool_temp")(temporal)

    s = AasistGraphReadout(name=f"{name}_readout_spec")(spectral)
    t = AasistGraphReadout(name=f"{name}_readout_temp")(temporal)
