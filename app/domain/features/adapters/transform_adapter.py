...
from app.domain.features.extractors.transform.transform_features import (
    TransformFeatureExtractor
)


class TransformExtractorWrapper(IFeatureExtractor):
    def __init__(self):
        self.extractor = TransformFeatureExtractor(sr=22050)
    ...
    def extract(
        ...
        try:
            ...
            audio_features = AudioFeatures(
                ...
                audio_metadata={"extractor": "transform"}
            )
            ...
            return ProcessingResult(
                ...
                errors=[f"Erro na extração de transformadas: {str(e)}"]
            )
    ...
    def get_extraction_params(self) -> Dict[str, Any]:
        sr = getattr(
            self.extractor, 'sr', getattr(self.extractor, 'sample_rate', 22050)
        )
        return {
            "sr": sr,
