...
class CrossValidationRequest(BaseModel):
    ...
    def validate_architecture(cls, v):
        return _validate_architecture_name(v)
...
def _validate_architecture_name(v: Optional[str]) -> Optional[str]:
    """Normaliza e valida um nome de arquitetura contra VALID_ARCHITECTURES.

    Compartilhado pelos `field_validator("architecture")` de
    CrossValidationRequest/TrainingRequest/ProfileCreate/ProfileUpdate/
    ProfileTrainRequest — antes cada um reimplementava a mesma checagem,
    e apenas as 3 últimas normalizavam "-"/" " para "_" (CrossValidationRequest
    e TrainingRequest rejeitavam nomes com hífen/espaço, ex.:
    "Hybrid-CNN-Transformer", que os outros 3 aceitavam).
    """
    if v is None:
        return v
    v_norm = v.lower().strip().replace("-", "_").replace(" ", "_")
    if v_norm not in VALID_ARCHITECTURES:
        raise ValueError(
...
def _validate_email_format(v: Optional[str]) -> Optional[str]:
    """Compartilhado por ProfileCreate/ProfileUpdate (`field_validator("email")`)."""
