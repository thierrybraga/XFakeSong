...
def build_pipeline():
    ...
    write_nb("pipeline/01_benchmark_tcc_full_pipeline.ipynb", [
        ...
        cmd = [
            sys.executable,
            str(ROOT / "scripts" / "benchmark" / "run_tcc_pipeline.py"),
            "--tcc-full-dataset",
            "--out", str(OUTPUT_DIR),
            "--npz", str(DATASET_NPZ),
        ]
    ...
    write_nb("pipeline/02_training_model.ipynb", [
        ...
        if DOWNLOAD_REAL:
            ...
            cmd = [
                sys.executable, str(ROOT / "scripts" / "benchmark" / "run_tcc_pipeline.py"),
                "--download", "--tier", TIER,
                "--archs", "SVM",
                "--epochs", "5",
                "--npz", str(REAL_NPZ),
        ...
        python scripts/dataset/download_datasets.py --fleurs --max-samples 300     # real (público)
        python scripts/dataset/download_datasets.py --brspeech --max-samples 600   # real+fake (pode pedir token)
        ```
        """),
        md("""
    ...
    write_nb("pipeline/04_all_architectures_full_benchmark.ipynb", [
        ...
        O comando abaixo usa `scripts/benchmark/run_tcc_pipeline.py` como orquestrador
