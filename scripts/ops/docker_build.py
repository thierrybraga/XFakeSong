#!/usr/bin/env python3
"""Helper de build/execucao dos perfis Docker do XFakeSong.

Encapsula os composes segmentados de ``docker/compose/`` em um unico CLI,
mapeando cada perfil (inference/train/benchmark x cpu/nvidia) para o par
(arquivo compose, servico) correto.

Uso:
    python scripts/ops/docker_build.py <perfil> config   # valida a config
    python scripts/ops/docker_build.py <perfil> build    # build da imagem
    python scripts/ops/docker_build.py <perfil> up       # sobe o servico
    python scripts/ops/docker_build.py <perfil> run      # execucao one-shot

Perfis: inference-cpu, inference-nvidia, train-cpu, train-nvidia,
benchmark-nvidia. Consumido pelo Makefile (``make build``/``docker-config``)
e pela CI.
"""
