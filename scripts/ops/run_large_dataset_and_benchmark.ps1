...
python scripts/ops/docker_build.py benchmark-nvidia build
...
docker compose -f docker/compose/benchmark.nvidia.yml --env-file .env run --rm benchmark `
    python scripts/dataset/build_dataset.py --tier large

Write-Host "== Auditoria do speaker_manifest =="
docker compose -f docker/compose/benchmark.nvidia.yml --env-file .env run --rm benchmark `
    python scripts/dataset/audit_speaker_manifest.py --scope active --min-identified-ratio 0.40

Write-Host "== Exportando NPZ =="
docker compose -f docker/compose/benchmark.nvidia.yml --env-file .env run --rm benchmark `
    python scripts/dataset/export_npz_from_splits.py --out $Dataset --sample-rate 16000 --duration-sec 5.0
