...
Exemplos:
  python scripts/benchmark/run_tcc_pipeline.py --smoke
  python scripts/benchmark/run_tcc_pipeline.py --smoke --model SVM
  python scripts/benchmark/run_tcc_pipeline.py --skip-download --target-per-class 500 --archs SVM RandomForest
  python scripts/benchmark/run_tcc_pipeline.py --skip-download --skip-preprocess --model AASIST
  python scripts/benchmark/run_tcc_pipeline.py --skip-download --skip-preprocess --archs SVM RandomForest
  python scripts/benchmark/run_tcc_pipeline.py --tcc-full-dataset --download --full-benchmark
"""
...
def main() -> int:
    ...
    parser.add_argument("--download", action="store_true", help="executa scripts/dataset/build_dataset.py antes do treino")
    ...
    if args.smoke:
        ...
        if args.download:
            cmd = [
                sys.executable,
                str(SCRIPTS_DIR / "dataset" / "build_dataset.py"),
            ]
            if args.tier:
            ...
            if args.skip_preprocess:
                ...
                _run(
                    [
                        sys.executable,
                        str(SCRIPTS_DIR / "dataset" / "preprocess_dataset.py"),
                        "--full",
                    ],
    ...
    bench_cmd = [
        sys.executable,
        str(SCRIPTS_DIR / "benchmark" / "run_benchmark.py"),
        "--dataset",
        str(npz_path),
        "--epochs",
        str(args.epochs),
