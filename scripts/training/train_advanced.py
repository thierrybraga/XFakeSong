...
Uso:
  python scripts/training/train_advanced.py                        # todas as arquiteturas
  python scripts/training/train_advanced.py --model conformer      # uma arquitetura
  python scripts/training/train_advanced.py --epochs 100           # epocas maximas
  python scripts/training/train_advanced.py --quick                # 20 epocas, debug rapido
"""
...
def prepare_splits(splits_dir: Path, datasets_dir: Path) -> bool:
    ...
    if real_count == 0 or fake_count == 0:
        logger.error(
            ...
            "Execute primeiro: python scripts/dataset/build_dataset.py"
        )
        return False
    ...
    result = subprocess.run(
        [sys.executable, str(BASE_DIR / "scripts" / "dataset" / "preprocess_dataset.py"),
...
def main():
    ...
    if real_train + fake_train < 20:
        logger.error(
            ...
            "Execute: python scripts/dataset/build_dataset.py"
        )
        sys.exit(1)
    ...
    logger.info("  python scripts/reporting/run_shap_analysis.py")
    logger.info("  python scripts/benchmark/robustness_test.py")
