...
#   bash scripts/training/retrain_wsl2.sh --check                  # só valida GPU/ambiente
#   bash scripts/training/retrain_wsl2.sh --indist                 # retreino in-distribution (14 arq.)
#   bash scripts/training/retrain_wsl2.sh --xgen fkvoice           # reteste cross-generator (P0.1)
#   bash scripts/training/retrain_wsl2.sh --group                  # split disjunto por fonte (P0.2)
...
run_bench () {  # $1 = sufixo do out, $2... = flags extras
  ...
  $PY scripts/benchmark/run_benchmark.py "${COMMON[@]}" --out "$out" "$@"
