...
- ``score_distributions_gat.png``: histogramas da pontuacao ``p_fake`` por
  classe verdadeira para AASIST e RawGAT-ST, descrevendo concentração e
  sobreposição sem atribuir causalidade à cabeça AM-Softmax.
...
def export_det_curves() -> Path:
    ...
    # CONVENÇÃO (corrigido em 2026-08-19): os eixos são FPR/FNR da convenção de
    # CLASSIFICAÇÃO, com a classe positiva = spoof — FPR é bonafide rejeitado e
    # FNR é spoof aceito. Rotulá-los $P_{fa}$/$P_{miss}$ INVERTIA os símbolos
    # em relação à convenção ASV/t-DCF declarada no texto, na qual
    # $P_{miss}^{cm}$ é justamente rejeitar bonafide.
