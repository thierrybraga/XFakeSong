...
def export(
    ...
    # POLITICA DE FORMATO GRAVADA NO ARTEFATO.
    #
    # `extract_window` aplica passa-baixas de 7,5 kHz, remocao de DC e
    # normalizacao de nivel (desde 2026-08-19), e o benchmark aplica a MESMA
    # correcao de novo quando recebe `--band-correction-hz` — que o compose
    # oficial passa por padrao. Sem registrar a politica no .npz nao havia como
    # detectar a dupla aplicacao: quem reexportasse o dataset com o comando
    # documentado em configs/dataset.yaml treinaria com o filtro em cascata,
    # que nao e o protocolo declarado. `benchmarks/data.py` le este bloco e
    # `run_benchmark` recusa a segunda aplicacao.
    from app.domain.features.benchmark_frontend import (
        BAND_CORRECTION_HZ,
        BAND_CORRECTION_RMS_DBFS,
        BAND_CORRECTION_TAPS,
        CORPUS_PEAK_CEILING_DBFS,
    )

    meta["band_correction"] = {
        "applied": True,
        "applied_by": "scripts/dataset/build_paired_pt_corpus.py::extract_window",
