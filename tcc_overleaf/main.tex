...
\begin{equation}
\label{eq:melspec}
MelSpec[m,k] = \log\!\left(\sum_{f}H_k[f]\cdot|STFT[m,f]|^2 +
\varepsilon\right)
\end{equation}
\noindent
em que $f$ indexa as \textit{bins} de frequência, $H_k[f]$ é o banco de $128$
filtros triangulares na escala Mel, $STFT$ é a transformada de Fourier de
curto termo com janela de \num{2048} amostras (\textit{n\_fft}) e
$\varepsilon$ evita $\log(0)$ na Equação~\ref{eq:melspec}. O
\textit{hop length} de \num{512} amostras determina a resolução temporal: para
as janelas de \SI{5}{\second} (\num{80000} amostras), o mapa resultante tem
dimensão $128 \times 157$.
...
do \textit{Random Forest} (\path{scripts/export_rf_feature_importance.py}). Agregando por família, os
...
(geradas por \texttt{scripts/export\_tcc\_extra\_figures.py}). A cabeça
...
\texttt{scripts/export\_tcc\_extra\_figures.py}). O platô de
...
\begin{itemize}
    ...
    \begin{quote}\footnotesize\ttfamily
    python scripts/run\_wavlm\_original\_benchmark.py --architecture wavlm\\
    --dataset app/datasets/benchmark\_audio\_raw\_balanced\_15k.npz\\
    python scripts/run\_wavlm\_original\_benchmark.py --architecture hubert\\
    --dataset app/datasets/benchmark\_audio\_raw\_balanced\_15k.npz
    \end{quote}
