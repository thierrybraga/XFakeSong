...
Forest} foram otimizados por validação cruzada. No conjunto de teste limpo, Res2Net e Conformer obtiveram
\num{99,69}\% de acurácia, seguidos por Audio Spectrogram Transformer
(\num{98,71}\%), \textit{Random Forest} (\num{98,18}\%) e RawNet2
(\num{97,38}\%). Sob ruído aditivo a \SI{10}{\decibel} de SNR, Conformer, AST
e Res2Net preservaram mais de \num{96}\% de acurácia, enquanto os
classificadores clássicos degradaram para \num{66}--\num{68}\% ---
...
\par\textbf{Escopo neste trabalho:} O SVM é treinado sobre o vetor tabular de \num{63} descritores (estatísticas temporais, MFCC e RASTA-PLP; Seção~\ref{sec:caracteristicas}) padronizado por \textit{z-score} --- e não sobre LFCC/CQCC, usados na literatura citada acima ---, com busca em grade por validação cruzada de \num{5} dobras sobre o \textit{kernel} (linear, RBF, polinomial), $C \in \{0{,}1;\, 1;\, 10;\, 100\}$ e $\gamma \in \{\texttt{scale},\, \texttt{auto},\, 10^{-3}\text{--}1\}$, seguida de reajuste final no conjunto de treino completo.
...
\SI{5}{\second} (\num{80000} amostras $\Rightarrow$ \textit{hop} de
\num{800}), resultando em mapas de dimensão fixa $100 \times 80$,
...
Cardinalidade por partição & \num{10500} (Tr), \num[group-minimum-digits=4]{2250} (Vl), \num[group-minimum-digits=4]{2250} (Te) \\
...
Particionamento de dados (\textit{Split}) & Razão 70/15/15 (\num{10500} treino, \num[group-minimum-digits=4]{2250} validação, \num[group-minimum-digits=4]{2250} teste) \\
...
(QP1). Com \num[group-minimum-digits=4]{2250} amostras de teste, uma acurácia de \num{99,69}\%
corresponde a cerca de \num{7} amostras mal classificadas, e a diferença de EER
entre Res2Net e Conformer equivale a aproximadamente duas amostras. Res2Net e
Conformer devem, portanto, ser tratados como \textbf{empate técnico}, e não
como ordenamento estrito entre primeiro e segundo colocados. O teste exato de
McNemar sobre as predições pareadas confirma essa leitura: dos \num[group-minimum-digits=4]{2250}
casos de teste, apenas \num{6} são acertados exclusivamente pelo Res2Net e
\num{6} exclusivamente pelo Conformer ($p = 1{,}0$); o mesmo vale para o par
SVM--CCT (\num{72} contra \num{71} desacordos, $p \approx 1{,}0$). Já a
diferença entre Conformer e AST é estatisticamente significativa (\num{5}
contra \num{27} desacordos, $p < 0{,}001$).
...
sobre vetor tabular. AASIST registra EER de \num{7,42}\%, RawGAT-ST de
\num{12,80}\%, WavLM de \num{15,24}\% e HuBERT de \num{11,29}\% --- cerca de
uma ordem de grandeza acima dos valores reportados pelos artigos originais no
...
(Conformer: \num{3} falsos positivos contra \num{4} falsos negativos; AST:
\num{14} contra \num{15}), mas o RawGAT-ST concentra os erros em falsos
positivos (\num{207} amostras \textit{bonafide} rejeitadas contra \num{86}
\textit{spoofs} aceitos), o que penaliza a usabilidade pela rejeição indevida
de usuários legítimos. Já AASIST e os \textit{backbones} SSL apresentam leve
excesso de falsos negativos (AASIST: \num{89} \textit{spoofs} aceitos contra
\num{80} rejeições indevidas; WavLM: \num{178} contra \num{168}) --- o erro
crítico no cenário de biometria em \textit{tandem}, em que aceitar um ataque
custa mais do que rejeitar um usuário genuíno
...
revelam uma dissociação entre acurácia em condição limpa e robustez sob
ruído. Os dois classificadores clássicos são os menos robustos do conjunto:
SVM cai a \num{66,44}\% e Random Forest a \num{68,04}\% a \SI{10}{\decibel},
apesar de figurarem entre os melhores no conjunto limpo (\num{96,00}\% e
\num{98,18}\%, respectivamente). Em contraste, Conformer (\num{98,44}\% a
10\,dB), AST (\num{97,38}\%) e Res2Net (\num{96,18}\%) são os mais robustos,
seguidos por RawNet2 (\num{90,80}\%), AASIST (\num{88,93}\%), RawGAT-ST
(\num{82,93}\%), CCT (\num{81,20}\%), HuBERT Original (\num{80,98}\%) e WavLM
Original (\num{75,91}\%) --- nenhum deles próximo do acaso (\num{50}\%).

Essa dissociação é o resultado analiticamente mais relevante deste
\textit{benchmark} (QP2): acurácia em distribuição fechada é um indicador
insuficiente de qualidade, e a robustez varia por mais de
\num{30} pontos percentuais entre os extremos do recorte, sem relação
monotônica com a família arquitetural. SVM e Random Forest, mesmo com
RASTA-PLP no vetor tabular --- incorporado
justamente para reduzir sensibilidade a variações lentas de canal --- e com
\textit{augmentation} de ruído (\texttt{classical\_noise\_augmentation}),
permanecem os mais frágeis, sugerindo que um vetor de \num{63} descritores
agregados tem expressividade estruturalmente limitada sob degradação do
sinal. Tampouco a família espectral garante robustez por si só: o CCT,
também espectral, cai a \num{81,20}\% a \SI{10}{\decibel} --- abaixo do
RawGAT-ST (\num{82,93}\%, áudio bruto) --- indicando que a capacidade do
modelo ($\approx$\num{1,8}~milhão de parâmetros, o menor entre as redes
espectrais) também modula o resultado. Conformer, AST e Res2Net combinam
representações tempo-frequência densas com capacidade suficiente, e são os
únicos que preservam desempenho quase intacto em todos os níveis testados.
...
Primeiro, há uma inversão notável entre AASIST e RawGAT-ST: o AASIST tem EER
cerca de \num{1,7} vez menor (\num{7,42}\% contra \num{12,80}\%), mas AUC-ROC
\textit{inferior} (\num{0,926} contra \num{0,951}). Como a AUC integra o
desempenho sobre todos os limiares e o EER captura um único ponto de operação,
essa combinação indica distribuições de pontuação com formas distintas ---
confirmadas empiricamente na Figura~\ref{fig:score_distributions_gat}: a
cabeça AM-Softmax do AASIST, por maximizar margem angular, satura as
pontuações nos extremos do intervalo (\num{99,9}\% dos \textit{scores} abaixo
de $0{,}05$ ou acima de $0{,}95$, contra \num{46,7}\% no RawGAT-ST),
produzindo excelente separação no ponto de equilíbrio, mas uma cauda de
\textit{spoofs} com pontuação extrema no lado incorreto que degrada a área
global sob a curva. Esse efeito é visível na curva DET do AASIST
(Figura~\ref{fig:benchmark_det_curves}) como um platô de
$P_{miss} \approx 7\%$, insensível ao limiar. Na prática, o AASIST é
preferível quando se opera próximo ao EER, e a comparação por AUC isolada
subestimaria seu valor operacional.
...
AM-Softmax do AASIST concentra \num{99,9}\% dos \textit{scores} nos extremos
do intervalo.}
...
\texttt{scripts/reporting/export\_tcc\_extra\_figures.py}). O platô de
$P_{miss} \approx 7\%$ do AASIST evidencia a cauda de \textit{spoofs} com
pontuação saturada no extremo incorreto.}
\label{fig:benchmark_det_curves}
\end{figure}

Segundo, o $t$-DCF$^\ast$ penaliza os \textit{backbones} SSL de forma
desproporcional ao EER: WavLM Original registra \num{0,7876} e HuBERT Original
\num{0,6427}, contra \num{0,0076} do Conformer --- razões muito maiores que as
respectivas razões de EER. Isso ocorre porque a parametrização adotada
(ASVspoof 2019 LA, com $C_{\text{fa}} = 10 \gg C_{\text{miss}} = 1$) pondera
fortemente a região de baixa taxa de falso-alarme, onde as distribuições de
pontuação dos SSL congelados são menos separáveis. Para o cenário de
verificação de locutor em \textit{tandem} que a métrica modela
(Seção~\ref{sec:biometria_voz}), esses modelos são, portanto, menos adequados
do que o EER isolado sugeriria --- ilustrando por que o protocolo reporta as
...
\num{8,48}\,ms do WavLM Original e os \num{6,41}\,ms do HuBERT Original, por
...
acurácia limpa de \num{86,98}\% e robustez de \num{82,93}\% a
\SI{10}{\decibel}; SVM alia latência mínima em CPU (\num{1,02}\,ms) a acurácia
competitiva em condição limpa (\num{96,00}\%), mas é, junto com o Random
Forest, o modelo menos robusto sob ruído, tornando-se atrativo apenas para
cenários em que a robustez a ruído não é prioritária. Conformer, o modelo mais
robusto sob ruído, ocupa posição intermediária de custo (\num{15,9}~milhões de
parâmetros, \num{61,27}\,MB, \num{59,21}\,ms em CPU), enquanto AST, segundo em
robustez, é o modelo mais pesado do conjunto (\num{87,4}~milhões de
parâmetros, \num{333,91}\,MB). Não há, portanto, um único modelo dominante em
todas as dimensões: a escolha depende de qual restrição --- latência, memória
ou robustez --- é prioritária na aplicação-alvo.
...
inferiores a \num{2}\%. A principal exceção é o CCT (queda de \num{3,78}\%),
com flutuação moderada de validação ao longo do treino. Vários modelos atingem
o pico de validação relativamente cedo --- WavLM Original na época \num{10},
Conformer e HuBERT Original na época \num{17}, RawGAT-ST na época \num{21} ---
sem que isso, isoladamente, indique instabilidade: a queda de RawGAT-ST entre
pico e valor final é controlada (\num{1,87}\%). O Conformer atinge
\num{100,00}\% de acurácia de validação já na época \num{17}, sinal de que a
tarefa de validação é, nesse ponto, quase trivialmente separável. Esse
resultado reforça a leitura cautelosa sobre vazamento de domínio discutida
acima, em vez de ser apenas um indicador positivo de convergência.
...
como estatisticamente significativas. Uma régua útil é o intervalo de
confiança de Wilson a \num{95}\% para a acurácia, que quantifica apenas a
incerteza amostral do conjunto de teste ($n = \num[group-minimum-digits=4]{2250}$): ele varia de
$\pm 0{,}24$ ponto percentual nos modelos de topo (\num{99,69}\% $\rightarrow$
$[\num{99,36}\%;\, \num{99,85}\%]$) a $\pm 1{,}5$ ponto nos de menor acurácia
(\num{84,67}\% $\rightarrow$ $[\num{83,12}\%;\, \num{86,10}\%]$). Sob essa
régua --- e sob o teste exato de McNemar pareado, que resulta em
$p \approx 1{,}0$ para os pares Res2Net--Conformer e SVM--CCT ---, o empate
SVM--CCT no conjunto limpo (\num{96,00}\% vs.\ \num{96,04}\%)
é indistinguível, enquanto as diferenças de robustez a \SI{10}{\decibel}
discutidas acima --- superiores a \num{10} pontos percentuais entre grupos ---
...
Forest} (\num{98,18}\%). Esse ordenamento (QP1) confirma que representações
...
Contudo, o achado analiticamente mais relevante (QP2) emerge do contraste
entre desempenho limpo e robustez sob ruído. Enquanto Conformer e AST
preservaram acurácia acima de \num{97}\% a \SI{10}{\decibel} de SNR, os dois
classificadores clássicos --- SVM e \textit{Random Forest}, com acurácia limpa
competitiva (\num{96,00}\% e \num{98,18}\%) --- degradaram para \num{66,44}\%
e \num{68,04}\%, tornando-se os modelos menos robustos do conjunto, enquanto
RawGAT-ST, AASIST, WavLM Original e HuBERT Original preservaram entre
aproximadamente \num{76}\% e \num{89}\% de acurácia a \SI{10}{\decibel}
(Seção~\ref{sec:analise_resultados}). Essa dissociação evidencia que a
acurácia em distribuição fechada é um indicador insuficiente de qualidade:
SVM e \textit{Random Forest} permanecem frágeis apesar do treino com ruído,
indicando que descritores estatísticos agregados são estruturalmente menos
expressivos sob degradação do sinal do que representações aprendidas, e o
CCT mostra que a capacidade do modelo também modula o resultado.
...
aponta o ajuste fino integral como etapa necessária para explorar seu
potencial. Notavelmente, ambos os modelos preservam robustez sólida sob
ruído (\num{75,91}\% e \num{80,98}\% a \SI{10}{\decibel}), indicando que a
limitação de desempenho limpo é específica da capacidade representacional de
um \textit{backbone} congelado, e não um sintoma de fragilidade sob
degradação do sinal.
...
oferece o melhor compromisso global (maior robustez, custo intermediário de
\num{61}\,MB e \num{59}\,ms em CPU), o AST troca \num{334}\,MB de artefato pelo
segundo melhor desempenho sob ruído, e o SVM permanece a opção de latência
mínima em CPU (\num{1}\,ms) para cenários em que a robustez a ruído não é
crítica --- lembrando que as latências foram medidas em dispositivos distintos
por família e valem como ordens de grandeza
