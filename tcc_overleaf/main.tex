...
\subsubsection{Construção da Base Experimental Balanceada}
...
único de avaliação.\footnote{O arquivo consolidado do \textit{benchmark}
agrupa amostras rotuladas como reais e sintéticas em uma base operacional
única. Ele \emph{não} implica composição majoritariamente em português
brasileiro: a proporção por idioma ainda não foi consolidada no manifesto
atual (ver Seção~\ref{sec:ameacas_validade}), e essa verificação é
pré-requisito para qualquer afirmação sobre cobertura linguística.}
...
\begin{table}[H]
\centering
\caption{Resumo da base experimental balanceada usada no \textit{benchmark}.}
...
Os modelos neurais foram treinados com orçamento máximo de 100 épocas,
semente 42, divisão estratificada, \textit{early stopping} e salvamento de
melhor \textit{checkpoint}. Assim, a coluna de treino nas tabelas indica o
orçamento configurado, enquanto o histórico registra as épocas efetivamente
executadas antes da parada antecipada. O \textit{batch size} padrão foi 32,
reduzido para 16 em
...
divisão estratificada, orçamento de até 100 épocas para modelos neurais e validação
...
Semente & 42 \\
Treino neural & até 100 épocas, \textit{early stopping} e melhor \textit{checkpoint} salvo \\
...
validação, a validação final e a queda de desempenho ao longo do orçamento
máximo de 100 épocas.

\begin{table}[H]
\centering
\caption{Estabilidade dos modelos neurais durante o orçamento máximo de 100 épocas.}
...
\begin{itemize}
    ...
    \item \textbf{Validade externa}: os resultados refletem a base consolidada
    de 15.000 amostras usada neste \textit{benchmark}. A generalização para
    outros idiomas, microfones, codecs,
    ...
    \item \textbf{Idioma}: o manifesto atual da base consolidada não documenta
    a proporção de amostras em português brasileiro. Um sistema treinado
    predominantemente em inglês pode ter viés de idioma que inflacione métricas
    e falhe em cenários reais de uso no Brasil.
\end{itemize}
...
\begin{itemize}
    ...
    \textit{dropout} maior e \textit{early stopping} guiado apenas por
    validação, preservando o teste final como avaliação independente.
    ...
    \item Documentar metadados completos da base consolidada e de cada fonte
    de áudio (falantes, idioma, licenças, durações, verificação de duplicatas).
...
O preset final do \textit{benchmark} utiliza orçamento máximo de 100 épocas
para modelos neurais, batch adaptado ao consumo de VRAM, precisão mista quando
segura, \textit{early stopping} e salvamento do melhor \textit{checkpoint}.
Modelos SSL originais utilizam seus
...
\midrule
WavLM/HuBERT & Áudio bruto \SI{16}{\kilo\hertz} & até 100 épocas + cabeça supervisionada & \textit{Backbone} original salvo localmente \\
RawNet2 & PCM $80{.}000\times1$ & até 100 épocas, GPU, GRU + SincNet & Convergente no preset GPU \\
AASIST/RawGAT-ST & Espectro-temporal & até 100 épocas, atenção em grafos & Feedback e checkpoints por época \\
Conformer/Hybrid & Espectrograma & até 100 épocas, Adam/AdamW & Conformer estável; Hybrid exige \textit{checkpoint} \\
Spectrogram Transformer & Espectrograma & até 100 épocas & Acurácia de 97,69\%; validação instável \\
EfficientNet-LSTM & Espectrograma 2D & até 100 épocas, transferência parcial & Boa convergência, maior latência \\
MultiscaleCNN/Ensemble & Características espectrais & até 100 épocas & Acurácia elevada na base limpa \\
...
\begin{table}[H]
\centering
\caption{Módulos reais do XFakeSong referenciados pelo \textit{benchmark}.}
...
Neste contexto, XFakeSong é o nome do sistema de detecção de áudio sintético
