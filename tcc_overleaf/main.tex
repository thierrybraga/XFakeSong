...
\end{equation}
\noindent
em que $L_{target} = -23$\,LUFS é o nível alvo de \textit{loudness} na
Equação~\ref{eq:agc}, aplicado como aproximação por normalização RMS --- a
conversão LUFS exata exigiria ponderação \textit{K-weighting}
(ITU-R BS.1770), não implementada nesta etapa.
...
\end{equation}
\noindent
em que $\theta_{VAD} = -25$\,dB é o limiar de energia, $H$ é o
\textit{hop length} (passo, em amostras, entre janelas consecutivas de
\SI{25}{\milli\second}; a \SI{16}{\kilo\hertz}, $H$ corresponde a
\SI{10}{\milli\second}), $D[m]$ é a duração do segmento contíguo de fala e
$D_{min} = 0{,}3$\,s é a duração mínima para manter um segmento
...
\end{equation}
\noindent
em que $H_k[n]$ é o banco de $128$ filtros triangulares na escala Mel,
$STFT$ é a transformada de Fourier de curto termo com janela de
\num{2048} amostras (\textit{n\_fft}) e $\varepsilon$ evita $\log(0)$ na
Equação~\ref{eq:melspec}. O \textit{hop length} de \num{512} amostras
determina a resolução temporal do espectrograma resultante.
...
protocolo padronizado com teto de até \num{120} épocas, parada antecipada
monitorando a perda de validação e salvamento automatizado do melhor
\textit{checkpoint} com restauração dos melhores pesos. O número de épocas efetivamente treinadas por
...
\textit{benchmark}: acurácia em distribuição fechada é um indicador
insuficiente de qualidade, e a robustez varia por um fator superior a
...
O preset final do \textit{benchmark} utiliza teto de até 120 épocas para os
modelos neurais, com parada antecipada por perda de validação, batch adaptado
ao consumo de VRAM, precisão mista quando segura e salvamento do melhor
\textit{checkpoint} com restauração dos melhores pesos.
