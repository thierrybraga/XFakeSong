...
qualidade e que a exposição a ruído durante o treinamento é fator de primeira
ordem para a robustez. Os resultados confirmam a funcionalidade do
...
insufficient quality indicator and that noise exposure during training is a
first-order factor for robustness. The results confirm the pipeline's ability
...
voz. Relatórios do setor indicam expansão global sustentada do mercado, puxada
...
modelo é treinado sob o mesmo protocolo padronizado
...
inferência (\subref{fig:pipeline_pred}). O limiar de decisão $\theta$ é
$0{,}5$ por padrão; para WavLM Original e HuBERT Original, é calibrado em
validação com ruído (Seção~\ref{sec:metricas}).}
\label{fig:pipeline}
\end{figure}
...
(Figura~\ref{fig:rf_feature_importance}). Cabe uma ressalva de escopo: o repositório também implementa
...
($\theta \approx 0{,}49$ e $\theta \approx 0{,}48$, respectivamente,
persistidos junto aos artefatos). Essa assimetria é declarada por
...
protocolo padronizado com teto de \num{100} épocas (\num{120} para os quatro
modelos retreinados após a correção descrita na
Seção~\ref{sec:analise_resultados}), parada antecipada monitorando a perda de
validação e salvamento automatizado do melhor \textit{checkpoint} com
restauração dos melhores pesos. O número de épocas efetivamente treinadas por
modelo --- tipicamente inferior ao teto por ação da parada antecipada --- é
reportado na coluna \textit{Treino} da
Tabela~\ref{tab:resultados_consolidados}. Durante o processo, coletaram-se
históricos de perda (\textit{loss}) e acurácia, matrizes de confusão, curvas
ROC e de Precisão-Revocação (\textit{Precision-Recall}), métricas por classe,
tamanho em disco e latência de inferência. Todos os experimentos usam semente
aleatória fixa ($42$) para particionamento e inicialização, registrada nos
...
\subsubsection*{Nota Metodológica: Correção de Treinamento}

Uma rodada preliminar deste \textit{benchmark} identificou dois problemas
distintos nos quatro modelos discutidos a seguir. Primeiro, o \textit{runner}
de treinamento de WavLM Original e HuBERT Original não expunha a cabeça de
classificação a exemplos ruidosos durante o treino --- apesar de a avaliação de
robustez injetar AWGN em \num{30}/\num{20}/\SI{10}{\decibel} --- e não aplicava
parada antecipada nem calibração do limiar de decisão. Segundo, um artefato de
infraestrutura de execução fez com que um retreino inicial de RawGAT-ST usasse
hiperparâmetros desatualizados (taxa de aprendizado, \textit{dropout} e
regularização $L_2$ anteriores ao ajuste já presente no código), repetindo o
padrão de \textit{overfitting}/divergência de uma rodada de diagnóstico
anterior; AASIST, por sua vez, já treinava com \textit{augmentation} de ruído,
mas com regularização excessiva e taxa de aprendizado conservadora,
resultando em subajuste e colapso de \textit{recall} sob ruído. Os quatro
modelos foram retreinados com as correções aplicadas --- \textit{augmentation}
de ruído casada com o teste, parada antecipada com restauração dos melhores
pesos e calibração do limiar de decisão sob ruído para os dois modelos SSL;
hiperparâmetros sincronizados e regularização revista para RawGAT-ST e AASIST
--- sob o mesmo particionamento estratificado 70/15/15 dos demais \num{7}
modelos, preservando a comparabilidade da
Tabela~\ref{tab:resultados_consolidados}. Os números discutidos a seguir já
refletem essas correções.
...
para essas arquiteturas. Note-se que o EER de WavLM e HuBERT no conjunto limpo é
ligeiramente superior ao de uma versão preliminar deste treinamento sem
\textit{augmentation} de ruído: o ajuste que elimina o colapso de robustez sob
ruído (discutido a seguir) tem como custo uma perda marginal de separabilidade
em condição limpa, um \textit{trade-off} esperado ao expor o classificador a
exemplos ruidosos durante o treino.
...
(Seção~\ref{sec:caracteristicas}), extraída do \textit{Random Forest} treinado
--- sem retreino, diretamente do artefato do \textit{benchmark}
(\path{scripts/export_rf_feature_importance.py}). Agregando por família, os
...
revelam uma dissociação entre acurácia em condição limpa e robustez sob ruído,
embora de forma distinta da observada antes da correção descrita na nota
metodológica acima. Os dois classificadores clássicos tornam-se, nesta versão,
os menos robustos do conjunto: SVM cai a \num{66,44}\% e Random Forest a
\num{68,04}\% a \SI{10}{\decibel}, apesar de figurarem entre os melhores no
conjunto limpo (\num{96,00}\% e \num{98,18}\%, respectivamente). Em contraste,
Conformer (\num{98,44}\% a 10\,dB), AST (\num{97,38}\%) e Res2Net
(\num{96,18}\%) permanecem os mais robustos, seguidos por RawNet2
(\num{90,80}\%), AASIST (\num{88,93}\%), RawGAT-ST (\num{82,93}\%), CCT
(\num{81,20}\%), HuBERT Original (\num{80,98}\%) e WavLM Original
(\num{75,91}\%) --- nenhum deles próximo do acaso (\num{50}\%).

Essa dissociação permanece o resultado analiticamente mais relevante deste
\textit{benchmark}, mas com leitura mais precisa do que a de versões
anteriores: acurácia em distribuição fechada é um indicador insuficiente de
qualidade. A evidência mais direta sobre o papel do regime de treino vem do
experimento natural embutido na correção descrita na nota metodológica: WavLM
Original e HuBERT Original, com \textbf{arquitetura idêntica} antes e depois,
ganharam respectivamente $+23{,}3$ e $+30{,}3$ pontos percentuais de acurácia
a \SI{10}{\decibel} apenas ao incluir exemplos ruidosos e calibração no
treinamento da cabeça de classificação --- o que sugere que a exposição a
ruído compatível com o teste é fator de primeira ordem para a robustez. O
fator não é, porém, único: SVM e Random Forest já treinam com ruído
(\texttt{classical\_noise\_augmentation}) e com RASTA-PLP no vetor tabular ---
incorporado justamente para reduzir sensibilidade a variações lentas de canal
--- e ainda assim permanecem os mais frágeis, sugerindo que um vetor de
\num{63} descritores agregados tem expressividade estruturalmente limitada sob
degradação do sinal. Tampouco a família espectral garante robustez por si só:
o CCT, também espectral e treinado com \textit{augmentation}, cai a
\num{81,20}\% a \SI{10}{\decibel} --- abaixo do RawGAT-ST
(\num{82,93}\%, áudio bruto) --- indicando que capacidade do modelo
($\approx$\num{1,8}~milhão de parâmetros, o menor entre as redes espectrais)
também modula o resultado. Conformer, AST e Res2Net combinam representações
tempo-frequência densas, capacidade suficiente e treino com ruído, e são os
...
quase duas vezes menor (\num{7,42}\% contra \num{12,80}\%), mas AUC-ROC
\textit{inferior} (\num{0,926} contra \num{0,951}). Como a AUC integra o
desempenho sobre todos os limiares e o EER captura um único ponto de operação,
essa combinação indica distribuições de pontuação com formas distintas: a
cabeça AM-Softmax do AASIST, por maximizar margem angular, tende a saturar as
pontuações próximas dos extremos, produzindo excelente separação no ponto de
equilíbrio mas caudas que degradam a área global sob a curva. Na prática, o
AASIST é preferível quando se opera próximo ao EER, e a comparação por AUC
isolada subestimaria seu valor operacional.
...
classificador que explora correlações espúrias entre fonte, canal e rótulo. Com
a correção do treinamento sob ruído, a robustez deixou de discriminar
claramente os modelos de melhor desempenho limpo dos demais --- Res2Net,
Conformer, AST, RawNet2, AASIST, RawGAT-ST, CCT, HuBERT Original e WavLM
Original preservam todos entre \num{76}\% e \num{98}\% de acurácia a
\SI{10}{\decibel} --- o que remove um possível argumento indireto de robustez
...
compacto do conjunto (\num{241024} parâmetros, \num{1,02}\,MB) e, ainda assim,
mantém acurácia limpa de \num{86,98}\% e robustez de \num{82,93}\% a
\SI{10}{\decibel}, deixando de ser o modelo de pior desempenho geral; SVM alia
baixíssima latência (\num{1,02}\,ms) a acurácia competitiva em condição limpa
(\num{96,00}\%), mas é hoje, junto com o Random Forest, o modelo menos robusto
sob ruído, tornando-se atrativo apenas para cenários em que a robustez a ruído
não é prioritária. Conformer, o modelo mais robusto sob
...
pico e valor final é controlada (\num{1,87}\%), o que contrasta com a
instabilidade observada antes da correção de treinamento descrita na nota
metodológica acima, quando o pico ocorria já na época \num{6} e era seguido de
divergência acentuada (Seção~\ref{sec:analise_resultados}). O Conformer atinge
...
e \num{68,04}\%, tornando-se os modelos menos robustos do conjunto depois da
correção de um defeito de treinamento em RawGAT-ST, AASIST, WavLM Original e
HuBERT Original,
que passaram a preservar entre \num{76}\% e \num{89}\% de acurácia a
\SI{10}{\decibel} (Seção~\ref{sec:analise_resultados}). Essa dissociação
evidencia que a acurácia em distribuição fechada é um indicador insuficiente
de qualidade. Os retreinos de WavLM e HuBERT --- mesma arquitetura, mudando
apenas o regime de treino --- sugerem que a exposição a ruído compatível com o
teste é fator de primeira ordem para a robustez, embora não único: SVM e
\textit{Random Forest} permanecem frágeis mesmo treinando com ruído, indicando
que descritores estatísticos agregados são estruturalmente menos expressivos
sob degradação do sinal do que representações aprendidas, e o CCT mostra que
capacidade do modelo também modula o resultado.
...
potencial. Notavelmente, ambos os modelos passaram a preservar robustez sólida
sob ruído (\num{75,91}\% e \num{80,98}\% a \SI{10}{\decibel}) uma vez que o
treinamento da cabeça de classificação passou a incluir exemplos ruidosos,
indicando que a limitação de desempenho limpo é específica da capacidade
representacional de um \textit{backbone} congelado, e não um sintoma de
fragilidade sob degradação do sinal.

Sob a ótica de implantação, nenhum modelo domina simultaneamente acurácia,
robustez, latência e memória (Seção~\ref{sec:analise_resultados}): o Conformer
oferece o melhor compromisso global (maior robustez, custo intermediário de
\num{61}\,MB e \num{59}\,ms), o AST troca \num{334}\,MB de artefato pelo
segundo melhor desempenho sob ruído, e o SVM permanece a opção de latência
mínima (\num{1}\,ms) para cenários em que a robustez a ruído não é crítica. A
escolha do detector deve, portanto, partir das restrições da aplicação-alvo,
não de um ranking único.
...
a uniformização da calibração para todos os modelos eliminaria.
...
incorporar métricas de calibração e explicabilidade local (SHAP, GradCAM) e
conduzir testes de robustez contra compressão, reverberação e ruído real de
canal, aproximando o protocolo experimental de cenários operacionais de
biometria de voz.
...
O preset final do \textit{benchmark} utiliza teto de 100 épocas para os
modelos neurais das rodadas iniciais e de 120 para os quatro retreinados
(Seção~\ref{sec:analise_resultados}), com parada antecipada por perda de
validação, batch adaptado ao consumo de VRAM, precisão mista quando segura e
salvamento do melhor \textit{checkpoint} com restauração dos melhores pesos.
...
\midrule
WavLM Original & Áudio bruto 16\,kHz & Até 120 épocas + cabeça supervisionada, parada antecipada, \textit{augmentation} de ruído & \textit{Backbone} SSL pré-treinado, congelado \\
HuBERT Original & Áudio bruto 16\,kHz & Até 120 épocas + cabeça supervisionada, parada antecipada, \textit{augmentation} de ruído & Representações SSL por unidades ocultas, \textit{backbone} congelado \\
RawNet2 & PCM $80.000\times1$ & 100 épocas, GPU, GRU + SincNet & Convergente no preset GPU \\
AASIST/RawGAT-ST & Áudio bruto & Até 120 épocas, atenção em grafos, parada antecipada & Grafos espectrais e temporais derivados internamente \\
...
Res2Net & Espectrograma Mel & 100 épocas & Topologia multi-escala \\
...
\begin{itemize}
    ...
    \end{quote}
    \textit{Nota: \texttt{--full} cobre \num{9} dos \num{11} modelos deste
    trabalho, excluindo WavLM Original e HuBERT Original (executados por um
    \textit{runner} PyTorch/\texttt{transformers} dedicado, com backbone SSL
    real). Para reproduzir os dois modelos SSL:}
    \begin{quote}\footnotesize\ttfamily
