...
\usepackage{lmodern}
\usepackage{graphicx}
% MELHORIA: graphicspath removido — todos os \includegraphics já usam o
...
\hypersetup{
    ...
    % MELHORIA: palavras-chave em português, alinhadas ao resumo
...
%  FOLHA DE APROVAÇÃO
% ------------------------------------------------------------------
\pagestyle{empty}   % MELHORIA: garantir que folha de aprovação não herde cabeçalho
\begin{center}
...
\newpage
\pagestyle{fancy}   % MELHORIA: retomar cabeçalho apenas a partir daqui
...
consolidado, Conformer e Sonic Sleuth\footnote{Sonic Sleuth é
o nome interno dado pelo autor ao modelo híbrido de características clássicas
(LFCC, MFCC e CQT) com rede neural compacta desenvolvido neste trabalho.}
obtiveram 100,00\% de acurácia e EER de 0,00\% no conjunto de teste limpo,
seguidos por Hybrid CNN-Transformer (99,96\%), MultiscaleCNN (99,73\%), SVM
(99,02\%) e Random Forest (98,18\%). O Spectrogram Transformer foi a principal
arquitetura com instabilidade observada: alcançou 98,00\% de validação na época
11, mas terminou com 71,51\% de acurácia no teste, indicando necessidade de
restauração rigorosa do melhor \textit{checkpoint} e novo ajuste de
hiperparâmetros. Os resultados confirmam a funcionalidade do \textit{pipeline}
...
\vspace{0.4cm}
% MELHORIA: palavras-chave normalizadas por ponto-e-vírgula (NBR 6028:2021)
...
cross-validation. Under the consolidated protocol, Conformer and Sonic Sleuth
achieved 100.00\%
...
% MELHORIA: menção às implicações éticas já na introdução, retomadas na conclusão
...
% MELHORIA: nota sobre risco de uso dual logo na introdução
...
% MELHORIA: pipeline dividido em duas subfiguras para melhor legibilidade
\begin{figure}[H]
\centering
...
\node (fake)   [resultfake, below right=1.1cm and 0.9cm of dec] {FAKE};
...
% ------------------------------------------------------------------
\subsection{Dataset e Protocolo Experimental}
\label{sec:dataset_protocolo}
% MELHORIA: subseção própria para dataset/protocolo — separado de metodologia geral
% ------------------------------------------------------------------
...
\begin{itemize}
    ...
    exportados para um arquivo NumPy único. O manifesto do pipeline registra
    caminho de origem, parâmetros de áudio, contagens por classe, semente e
    partições.

    % MELHORIA: limitação documental tornada explícita e destacada
    ...
    % MELHORIA: nota explícita sobre risco de vazamento por falante
...
% MELHORIA: fonte explícita mínima de 9pt (verificar com \resizebox)
\resizebox{\textwidth}{!}{%
...
\emph{Esses valores indicam separação perfeita no recorte experimental
...
Em seguida, Hybrid CNN-Transformer, MultiscaleCNN e SVM permaneceram acima
de 99\% de acurácia, indicando que tanto arquiteturas espectrais profundas
quanto baselines clássicos bem ajustados extraem sinais discriminativos fortes
no \textit{dataset} definido.

RawNet2, Ensemble, RawGAT-ST, AASIST, HuBERT original e EfficientNet-LSTM
também convergiram adequadamente, com acurácia entre 91,16\% e 96,36\%.

% MELHORIA: análise aprofundada do WavLM vs HuBERT
\begin{warningbox}{WavLM com desempenho inferior ao HuBERT: hipóteses}
WavLM (86,36\%) ficou abaixo de HuBERT (92,71\%) apesar de ser
...
final caiu para 71,51\% no teste. Isso sugere instabilidade de
otimização, sensibilidade a hiperparâmetros e necessidade de restaurar
obrigatoriamente o melhor \textit{checkpoint} para inferência.
...
\resizebox{\textwidth}{!}{%
% MELHORIA: coluna de desvio padrão de latência adicionada
...
\midrule
Máx.\ acurácia limpa & Conformer; Sonic Sleuth & 100,00\% e EER 0,00\% & Referência no benchmark \\
Melhor robustez a \SI{10}{\decibel} & Conformer & 94,18\% sob AWGN & Com ruído moderado \\
Equilíbrio neural leve & Hybrid CNN-Transformer & 99,96\%, 6,67 MB, 33,69 ms & Demo Gradio e API \\
Menor latência & SVM & 0,20 ms e 0,47 MB & Baseline rápido em CPU \\
Modelo compacto clássico & SVM; Random Forest & 99,02\% e 98,18\% & Comparação interpretável \\
Pesquisa SSL & HuBERT; WavLM & Backbones originais CUDA & Comparação com literatura \\
Risco de instabilidade & Spectrogram Transformer & 71,51\% pós-98\% em val. & Novo ajuste necessário \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
Conformer & Pronto (demo principal) & Melhor acurácia e maior robustez sob AWGN \\
Sonic Sleuth & Pronto (demo leve) & Acurácia máxima, artefato pequeno, baixa latência \\
Hybrid CNN-Transformer & Pronto (Gradio/API) & Melhor compromisso acurácia/tamanho/latência neural \\
MultiscaleCNN & Pronto (benchmark) & Alta acurácia; latência e tamanho maiores \\
SVM & Pronto (baseline rápido) & Excelente latência; frágil sob ruído \\
Random Forest & Baseline complementar & Bom desempenho clássico; mais pesado que SVM \\
RawNet2 & Funcional (raw-audio) & Boa convergência e desempenho intermediário-alto \\
Ensemble & Funcional (ressalva robustez) & Bom no limpo; queda sob AWGN \\
RawGAT-ST & Funcional (grafos) & Estável e relativamente robusto sob ruído \\
AASIST & Funcional (grafos) & Treino corrigido; desempenho consistente \\
HuBERT Original & Referência SSL & Backbone original viável; custo elevado \\
EfficientNet-LSTM & Funcional (não prioritário) & Maior latência; acurácia abaixo dos líderes \\
WavLM Original & Referência SSL experimental & Baixa latência medida; acurácia inferior ao HuBERT \\
Spectrogram Transformer & Requer novo ajuste & Instabilidade validação/teste \\
\bottomrule
\end{tabular}}
\end{table}
...
separação melhor em \SI{30}{\decibel}.

% MELHORIA: análise aprofundada da queda do SVM
O SVM apresentou comportamento particularmente frágil sob AWGN: embora tenha
alcançado 99,02\% no áudio limpo, caiu para 50,00\% em todos os níveis de
ruído testados --- equivalente a decisão aleatória binária. Essa queda abrupta
tem uma explicação direta: o SVM com kernel RBF aprende uma fronteira de
decisão ajustada à distribuição das características do conjunto limpo (médias,
variâncias e coeficientes espectrais padronizados). Ruído AWGN desloca
sistematicamente essas estatísticas: o centroide espectral se move para
frequências mais altas, a planura espectral aumenta e o HNR cai, fazendo com
que a distribuição de teste saia completamente da região de suporte do modelo.
Em trabalhos futuros, esse baseline deve ser reavaliado com:
(1)~extração de características robustas a ruído (e.g., RASTA-PLP ou MFCC
...
Spectrogram Transformer & 98,00\% & 11 & 72,98\% & 25,02\% & \textcolor{dangerred}{checkpoint obrigatório} \\
...
Spectrogram Transformer é o único caso crítico, com queda de 25,02 pontos
percentuais --- o principal candidato a novo ciclo de regularização, redução de
taxa de aprendizado, aumento de \textit{dropout} e \textit{weight decay}.
...
atraentes. Para estudo de representações modernas e comparação com a
literatura, WavLM, HuBERT, RawNet2, AASIST e RawGAT-ST são essenciais.
...
trabalho (AASIST: 6,36\%, RawNet2: 3,56\%) são plausíveis para uma base
menor e menos diversa, mas a comparação direta é inválida pela diferença de
dataset. Modelos SSL como WavLM e HuBERT representam o estado moderno de
aprendizado auto-supervisionado; seu desempenho inferior neste benchmark
(86--93\%) pode refletir subajuste de hiperparâmetros em uma base menor,
não uma limitação intrínseca da arquitetura.

O bom desempenho do Conformer e do Hybrid CNN-Transformer é coerente com a
hipótese de que a combinação de convolução local e atenção global captura
artefatos espectro-temporais de forma eficiente.
...
\begin{itemize}
    ...
    pequenas no topo do ranking (ex.: 99,73\% vs 99,96\%) não são
...
% MELHORIA: ética integrada à discussão e retomada na conclusão
...
da literatura adversarial e reavaliação periódica do pipeline contra novos
geradores.
...
\begin{itemize}
    ...
    validar diferenças de ranking entre arquiteturas próximas.
...
% MELHORIA: conclusão mais concisa, focada em implicações e limitações abertas
...
Os resultados confirmam forte desempenho de modelos espectrais e híbridos no
protocolo controlado, com destaque para Conformer, Sonic Sleuth, Hybrid
CNN-Transformer, MultiscaleCNN e os baselines clássicos otimizados. A
principal contribuição não é o ranking final em si, mas a consolidação de um
...
externas como ASVspoof e WaveFake; e (3)~a fragilidade do SVM e de modelos
SSL sob ruído AWGN, que compromete a robustez em cenários reais de campo.
...
\midrule
WavLM/HuBERT & Áudio bruto \SI{16}{\kilo\hertz} & 100 épocas + cabeça supervisionada & Backbone original salvo localmente \\
...
SVM & Características tabulares & GridSearchCV + SVC RBF & Modelo leve; frágil sob AWGN \\
...
rastreabilidade dos principais artefatos gerados.
...
Backbones SSL & \path{app/models/benchmark_final/<modelo>/} & Artefatos completos de WavLM/HuBERT \\
...
\begin{table}[H]
\centering
\caption{Módulos reais do XFakeSong\protect\footnotemark\ referenciados pelo benchmark.}
...
Treinamento & \path{app/domain/services/training_service.py} & Cria o modelo, aplica callbacks, treina e salva artefatos \\
...
pipeline, executar modelos isolados e gerar os artefatos do artigo.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Matrizes de confusão, parte 2: espectrais, Ensemble e baselines clássicos.}
