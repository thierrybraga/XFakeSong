...
def test_best_epoch_segue_o_checkpoint_monitor_declarado():
    """Com val_eer, `best_epoch` é o mínimo do EER — não o da perda.

    Cenário medido no Conformer da bateria corrigida: a perda tem mínimo na
    época 19 e o EER na 50. Enquanto o diagnóstico assumia val_loss, o artefato
    de um run selecionado por val_eer declarava como "melhor época" uma que o
    ModelCheckpoint nem havia salvo.
    """
    val_acc = [0.90] * 100
    val_loss = [1.0] * 100
    val_loss[18] = 0.10  # época 19
    val_eer = [0.05] * 100
    val_eer[49] = 0.01  # época 50
    hist = _history(val_acc, val_loss)
    hist["val_eer"] = val_eer

    por_eer = analyze_training_stability(
        hist, epochs_budget=100, checkpoint_monitor="val_eer"
    )
    assert por_eer["checkpoint_monitor"] == "val_eer"
    assert por_eer["best_epoch"] == 50
    # As duas séries continuam registradas lado a lado.
    assert por_eer["best_epoch_by_val_eer"] == 50
    assert por_eer["best_epoch_by_val_loss"] == 19
    assert por_eer["selection_criteria_gap_epochs"] == 31
    assert por_eer["epochs_after_best"] == 50

    por_loss = analyze_training_stability(
        hist, epochs_budget=100, checkpoint_monitor="val_loss"
    )
    assert por_loss["best_epoch"] == 19


def test_val_eer_ausente_cai_para_val_loss_sem_mentir():
    """Artefatos antigos e o runner SSL não têm a série; o critério REAL é a perda."""
    val_loss = [1.0] * 10
    val_loss[3] = 0.2
    report = analyze_training_stability(
        _history([0.9] * 10, val_loss), epochs_budget=10, checkpoint_monitor="val_eer"
    )
    assert report["checkpoint_monitor"] == "val_loss"
    assert report["best_epoch"] == 4
    assert "best_epoch_by_val_eer" not in report


def test_padrao_continua_sendo_val_loss():
    """Nenhum chamador antigo muda de comportamento por omitir o argumento."""
    val_loss = [1.0] * 10
    val_loss[6] = 0.1
    report = analyze_training_stability(_history([0.9] * 10, val_loss))
    assert report["checkpoint_monitor"] == "val_loss"
    ...
    return [
        {
            ...
            "eer": 0.01,
            "robustness": {
                "30": {"accuracy": 0.98, "auc_roc": 0.998, "eer": 0.02},
                "20": {"accuracy": 0.96, "auc_roc": 0.990, "eer": 0.03},
                "10": {"accuracy": 0.90, "auc_roc": 0.970, "eer": 0.06},
                "5": {"accuracy": 0.82, "auc_roc": 0.950, "eer": 0.12},
            },
        },
        {
            "model": "RawGAT-ST",
            ...
            "eer": 0.12,
            "robustness": {
                "30": {"accuracy": 0.87, "auc_roc": 0.93, "eer": 0.14},
                "20": {"accuracy": 0.85, "auc_roc": 0.91, "eer": 0.16},
                "10": {"accuracy": 0.77, "auc_roc": 0.87, "eer": 0.21},
                "5": {"accuracy": 0.76, "auc_roc": 0.86, "eer": 0.22},
            },
        },
    ]


def test_figuras_de_robustez_incluem_limiar_fixo_e_metricas_livres(tmp_path):
    """Acurácia sozinha não separa discriminação de deriva de limiar."""
    pytest.importorskip("matplotlib")
    image_module = pytest.importorskip("PIL.Image")
    from scripts.reporting import consolidate_results as consolidate

    rows = _rows_para_figuras()
    consolidate.fig_robustness(rows, tmp_path)
    consolidate.fig_robustness_threshold_free(rows, tmp_path)

    for name in (
        "benchmark_robustness.png",
        "benchmark_robustness_threshold_free.png",
    ):
        path = tmp_path / name
...
def test_figuras_det_e_distribuicoes_usam_scores_canonicos_e_300_dpi(tmp_path):
    """As avaliações de limiar e de distribuição não podem ficar órfãs."""
    pytest.importorskip("matplotlib")
    image_module = pytest.importorskip("PIL.Image")
    ...
    rows = [
        {"model": "AASIST", "slug": "aasist"},
        {"model": "RawGAT-ST", "slug": "rawgat_st"},
    ]
    y_true = np.asarray([0, 0, 0, 1, 1, 1], dtype=int)
    extras = {
        "aasist": {
            "y_test": y_true,
            "scores_clean": np.asarray([0.01, 0.07, 0.65, 0.72, 0.96, 0.99]),
        },
        "rawgat_st": {
            "y_test": y_true,
            "scores_clean": np.asarray([0.08, 0.22, 0.57, 0.45, 0.76, 0.91]),
        },
    }

    consolidate.fig_det_curves(rows, extras, tmp_path)
    consolidate.fig_score_distributions_gat(rows, extras, tmp_path)

    for name in ("benchmark_det_curves.png", "score_distributions_gat.png"):
        path = tmp_path / name
        assert path.is_file() and path.stat().st_size > 10_000
        with image_module.open(path) as image:
            dpi = image.info.get("dpi")
...
def test_fragmento_declara_sete_keras_e_duas_ssl():
    from scripts.reporting.update_tcc_latex import tables_fragment

    fragment = tables_fragment("", "", "", "", "", "figures")

    assert "sete entradas do caminho Keras" in fragment
    assert "WavLM Original e HuBERT Original" in fragment
    assert "nove entradas do caminho Keras" not in fragment
    assert "benchmark_robustness_threshold_free.png" in fragment
    assert "benchmark_det_curves.png" in fragment
    assert "score_distributions_gat.png" in fragment
    assert "rf_feature_importance.png" in fragment
    assert "benchmark_latency.png" not in fragment
    assert "confusion_matrices_article.png" not in fragment
...
# ─── Arquivamento do artefato substituído num [RERUN] ───────────────────────


def _sequential():
    """Importa o orquestrador sem executar o `main`."""
    import importlib.util

    caminho = _PROJECT_ROOT / "scripts" / "benchmark" / "run_models_sequential.py"
    spec = importlib.util.spec_from_file_location("_seq_runner", caminho)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_rerun_move_o_artefato_anterior_para_fora_do_caminho(tmp_path):
    """Um retreino não pode herdar o estado de treino do run que ele substitui.

    Observado em 2026-08-22 no `[RERUN]` do Conformer: o
    `ResumableModelCheckpoint` restaurou "melhor val_eer 0.013736" — a marca da
    época 50 do run ANTERIOR. Com `save_best_only`, uma execução que nunca
    superasse esse valor terminaria com o `history` novo e os PESOS velhos.
    """
    mod = _sequential()
    root = tmp_path / "run"
    model_dir = root / "conformer"
    modelos = model_dir / "architectures" / "conformer" / "models"
    modelos.mkdir(parents=True)
    (modelos / "best_checkpoint.weights.h5.best.json").write_text(
        '{"monitor": "val_eer", "best": 0.013736}', encoding="utf-8"
    )
    (model_dir / "results.json").write_text("{}", encoding="utf-8")
    backup = model_dir / "architectures" / "conformer" / "training_backup"
    backup.mkdir(parents=True)
    (backup / "training_metadata.json").write_text('{"epoch": 100}', encoding="utf-8")

    destino = mod._archive_superseded(model_dir, root, "abcdef1234567890")

    assert destino is not None
    assert not model_dir.exists(), "o diretório antigo precisa sair do caminho"
    # Movido, NÃO apagado: o artefato substituído é a evidência do defeito.
    assert (destino / "results.json").is_file()
    assert (
        destino / "architectures" / "conformer" / "models"
        / "best_checkpoint.weights.h5.best.json"
    ).is_file()
    assert (destino / "architectures" / "conformer" / "training_backup").is_dir()


def test_artefato_arquivado_sai_do_glob_da_consolidacao(tmp_path):
    """`_superseded/` não pode voltar à tabela como se fosse mais um modelo."""
    mod = _sequential()
    root = tmp_path / "run"
    model_dir = root / "conformer"
    ...
    mod._archive_superseded(model_dir, root, "abcdef1234567890")

    # Mesmo glob que `consolidate_results` e `backfill_artifact_metadata` usam.
    assert sorted(root.glob("*/results.json")) == []


def test_arquivamento_e_idempotente_com_o_mesmo_fingerprint(tmp_path):
    """Uma segunda tentativa não pode sobrescrever o artefato já arquivado."""
    mod = _sequential()
    root = tmp_path / "run"
    model_dir = root / "conformer"
    model_dir.mkdir(parents=True)
    (model_dir / "results.json").write_text('{"original": true}', encoding="utf-8")
    destino = mod._archive_superseded(model_dir, root, "abcdef1234567890")

    # Resto de uma execução abortada, com o MESMO fingerprint.
    model_dir.mkdir(parents=True)
    (model_dir / "results.json").write_text('{"sobra": true}', encoding="utf-8")
    de_novo = mod._archive_superseded(model_dir, root, "abcdef1234567890")

    assert de_novo == destino
    assert not model_dir.exists()
    assert '"original": true' in (destino / "results.json").read_text(encoding="utf-8")


def test_sem_diretorio_anterior_nao_faz_nada(tmp_path):
    mod = _sequential()
    root = tmp_path / "run"
    root.mkdir(parents=True)
    assert mod._archive_superseded(root / "inexistente", root, "abc12345") is None
