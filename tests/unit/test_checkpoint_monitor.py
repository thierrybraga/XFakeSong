...
def test_logits_crus_usam_log_odds_e_nao_a_coluna_1():
    """AASIST emite LOGITS, nao softmax -- e ordenar por `logit[1]` mente.

    Medido no artefato promovido (2026-08-19): faixa -37,8 a +41,1, soma das
    colunas com media 1,91 e desvio 4,51. Ordenar so pela coluna 1 devolvia
    EER 0,0025 contra 0,0050 pela log-odds -- OTIMISTA por um fator de 2. Nao
    quebrava nada; selecionava o checkpoint por uma metrica errada.

    Aqui as duas colunas variam de modo independente e a coluna 1 sozinha
    ordena AO CONTRARIO da log-odds, o que separa os dois criterios.
    """
    y = [0, 0, 1, 1]
    # col0 varia forte; col1 varia pouco e na direcao errada.
    logits = np.array(
        [[0.0, 2.0], [6.0, 1.5], [0.0, 1.0], [-6.0, 0.5]], dtype="float64"
    )
    # log-odds: -2, 4.5, 1, 6.5  -> separa {0,0} de {1,1}? nao perfeitamente,
    # mas a ORDEM difere da coluna 1 (2, 1.5, 1, 0.5), que e decrescente.
    cb = ValidationEER(validation_data=(np.zeros((4, 1)), np.asarray(y)))
    cb.set_model(_ModeloFalso(logits))
    logs: dict = {}
    cb.on_epoch_end(0, logs)
    ...
    def _eer(sc):
        ...
        ok = np.isfinite(thr)
        f, n = fpr[ok], fnr[ok]
        i = int(np.nanargmin(np.abs(f - n)))
        return float((f[i] + n[i]) / 2)

    esperado = _eer(logits[:, 1] - logits[:, 0])
    so_col1 = _eer(logits[:, 1])
    assert esperado != so_col1, "o caso de teste nao separa os dois criterios"
    assert logs["val_eer"] == pytest.approx(esperado, abs=1e-9), (
        "o callback ordenou por logit[1] em vez da log-odds"
    )


def test_softmax_nao_muda_com_a_log_odds():
    """Com softmax as colunas somam 1, entao `col1 - col0 = 2*col1 - 1` e
    transformacao monotonica -- o EER tem de ser o mesmo. E o que permite usar
    a diferenca nos DOIS casos sem detectar o tipo de saida."""
    y = [0, 0, 1, 1]
    p1 = np.array([0.1, 0.2, 0.8, 0.9])
    softmax = np.stack([1 - p1, p1], axis=1)

    cb = ValidationEER(validation_data=(np.zeros((4, 1)), np.asarray(y)))
    cb.set_model(_ModeloFalso(softmax))
    logs: dict = {}
    cb.on_epoch_end(0, logs)
    assert logs["val_eer"] == pytest.approx(0.0, abs=1e-9)
...
def test_validation_eer_prediz_em_lotes():
    ...
    from app.domain.models.training.trainer import ValidationEER, validation_eer
    ...
    # O cálculo mudou de lugar em 2026-08-22 (passou a ser compartilhado com a
    # restauração guardada de checkpoint), mas o lote precisa continuar
    # atravessando: o callback repassa o seu, e é `validation_eer` quem chama
    # o `predict`.
    assert "self.batch_size" in fonte, "o lote do callback não é repassado"
    assert "validation_eer(" in fonte
    fonte_calculo = inspect.getsource(validation_eer)
    assert "batch_size=" in fonte_calculo, "predict sem lote"
...
def test_o_monitor_chega_ate_a_restauracao_guardada():
    """Encanamento: o guard precisa RECEBER o monitor, não assumir val_loss.

    O defeito de 2026-08-22 não estava no guard nem no ModelCheckpoint, mas no
    trecho entre os dois: o `TrainingService` chamava a restauração sem dizer
    por qual métrica o checkpoint tinha sido eleito, e o default (`val_loss`)
    desfazia a seleção. Um teste de comportamento do guard sozinho continuava
    passando — como continuou, durante toda a bateria corrigida.
    """
    import inspect

    from app.domain.services.training_service import TrainingService

    assinatura = inspect.signature(TrainingService._guarded_checkpoint_restore)
    assert "monitor" in assinatura.parameters

    fonte = inspect.getsource(TrainingService.train_model)
    assert "_guarded_checkpoint_restore(" in fonte
    chamada = fonte[fonte.index("_guarded_checkpoint_restore(") :][:600]
    assert "monitor=" in chamada, (
        "a chamada precisa passar o monitor explicitamente; sem isso o guard "
        "cai no default val_loss e descarta a época eleita por val_eer"
    )


def test_o_desfecho_da_restauracao_chega_ao_artefato():
    """`weights_evaluated` distingue checkpoint eleito de pesos da última época.

    Sem este campo, um artefato produzido com o checkpoint descartado é
    indistinguível de um produzido corretamente — a informação existia só como
    um WARNING no meio do run.log.
    """
    import inspect

    from app.domain.services.training_service import TrainingService

    fonte_servico = inspect.getsource(TrainingService.train_model)
    assert '"checkpoint_restore"' in fonte_servico
    assert '"weights_evaluated"' in fonte_servico

    from benchmarks import runner as runner_mod

    fonte_runner = inspect.getsource(runner_mod._run_neural)
    assert 'train_data.get("checkpoint_restore")' in fonte_runner, (
        "o runner precisa LER o desfecho que o serviço grava"
    )
    assert '"checkpoint_restore": checkpoint_restore' in fonte_runner, (
        "e precisa ESCREVÊ-LO no fit_strategy do metrics.json"
    )
