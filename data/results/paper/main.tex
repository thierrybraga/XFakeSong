...
Forest} foram otimizados por validação cruzada. No conjunto de teste limpo, HuBERT e Conformer obtiveram
\num{99,87}\% e \num{99,82}\% de acurácia, seguidos por Res2Net e WavLM
(ambos \num{99,69}\%), SVM (\num{99,24}\%) e CCT (\num{99,20}\%). Sob ruído
aditivo a \SI{10}{\decibel} de SNR, WavLM, Conformer, Res2Net e HuBERT
preservaram mais de \num{96}\% de acurácia, enquanto AASIST e RawGAT-ST
degradaram para \num{88,7}\% e \num{84,0}\% ---
...
Figura~\ref{fig:benchmark_accuracy_auc} mostra maior desempenho para HuBERT e
Conformer, com \num{99,87}\% e \num{99,82}\% de acurácia no conjunto de teste
limpo (ambos com EER de \num{0,18}\%). Em seguida aparecem Res2Net e WavLM
(ambos \num{99,69}\%, EER de \num{0,36}\%), SVM (\num{99,24}\%) e CCT
(\num{99,20}\%), indicando que redes neurais profundas e classificadores
estatísticos otimizados extraem padrões discriminativos relevantes no cenário
experimental controlado
(QP1). Com \num[group-minimum-digits=4]{2250} amostras de teste, uma acurácia de \num{99,87}\%
corresponde a apenas \num{3} amostras mal classificadas, e a diferença entre
HuBERT e Conformer equivale a uma única amostra. HuBERT e
...
casos de teste, apenas \num{3} são acertados exclusivamente pelo Conformer e
\num{4} exclusivamente pelo HuBERT ($p = 1{,}0$); o mesmo vale para os pares
...
(SVM: \num{99,24}\% $\rightarrow$ \num{93,8}\%; Random Forest:
\num{97,82}\% $\rightarrow$ \num{92,4}\%), mesmo limitados ao vetor de
\num{63} descritores agregados. Segundo, a fragilidade relativa se
concentra nas redes de grafos sobre áudio bruto: RawGAT-ST perde
\num{9,6} pontos percentuais entre o conjunto limpo e \SI{10}{\decibel}
(\num{93,60}\% $\rightarrow$ \num{84,0}\%) e AASIST perde \num{6,3}
(\num{95,02}\% $\rightarrow$ \num{88,7}\%) --- as maiores quedas do
recorte, indicando que a janela curta de \SI{1}{\second} e a atenção em
...
acurácia limpa de \num{93,60}\% e robustez de \num{84,0}\% a
\SI{10}{\decibel}; SVM alia latência mínima em CPU (\num{0,28}\,ms) a
acurácia competitiva em condição limpa (\num{99,24}\%) e robustez de
\num{93,8}\% a \SI{10}{\decibel}, tornando-se uma opção atrativa para
...
épocas variadas --- Conformer na época \num{37}, RawGAT-ST na \num{42}, CCT
na \num{57}, Res2Net na \num{72}, WavLM e HuBERT Original na \num{81} ---
sem convergência precoce sob o protocolo com \textit{augmentation} de ruído
no treino. O Conformer atinge \num{99,87}\% de acurácia de validação, sinal
de que a tarefa de validação é, nesse ponto, quase trivialmente separável.
Esse resultado reforça a leitura cautelosa sobre vazamento de domínio
discutida acima, em vez de ser apenas um indicador positivo de convergência.
...
particionamento. Diferenças pequenas entre modelos próximos, como HuBERT e
Conformer (\num{99,87}\% vs.\ \num{99,82}\% de acurácia), não devem ser
...
$\pm 0{,}17$ ponto percentual no topo (\num{99,87}\% $\rightarrow$
$[\num{99,61}\%;\, \num{99,95}\%]$) a $\pm 1{,}0$ ponto no modelo de menor
acurácia (\num{93,60}\% $\rightarrow$ $[\num{92,51}\%;\, \num{94,54}\%]$).
Sob essa régua --- e sob o teste exato de McNemar pareado, que resulta em
$p \approx 1{,}0$ para os pares HuBERT--Conformer e SVM--CCT ---, o empate
...
No conjunto de teste limpo, os \textit{backbones} auto-supervisionados e as
arquiteturas espectro-temporais densas lideraram: HuBERT Original e Conformer
alcançaram \num{99,87}\% e \num{99,82}\% de acurácia (ambos com EER de
\num{0,18}\% e AUC-ROC $\approx 1{,}000$), seguidos por Res2Net e WavLM
Original (ambos \num{99,69}\%), SVM (\num{99,24}\%) e CCT (\num{99,20}\%).
Esse ordenamento (QP1) confirma que representações
tempo-frequência ricas, associadas a mecanismos de atenção ou convoluções
multi-escala --- e \textit{embeddings} SSL pré-treinados em larga escala,
lidos por soma ponderada de camadas --- capturam de modo eficaz os artefatos
de síntese presentes no \textit{corpus} avaliado.
...
congelados e com apenas a cabeça de classificação treinada, alcançaram o topo
do recorte (\num{99,69}\% e \num{99,87}\% no conjunto limpo; \num{98,8}\% e
\num{96,8}\% a \SI{10}{\decibel}) --- resultado obtido com a leitura por soma
ponderada das \num{13} camadas ocultas, \textit{pooling}
média$\oplus$desvio e janela de \SI{4}{\second}, em linha com o protocolo
SUPERB. Esse comportamento é consistente com a literatura sobre
