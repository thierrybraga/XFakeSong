...
> [../evaluation/final-ml-protocol.md](../evaluation/final-ml-protocol.md).

> Criado em 2026-06-26. Origem: análise crítica dos resultados consolidados
> (`data/results/tcc_consolidated/benchmark_summary.json`, corrida `20260626`).
> Relacionado: [retraining-plan.md](retraining-plan.md),
> [../evaluation/retraining-wsl2.md](../evaluation/retraining-wsl2.md), [../evaluation/experimental-study.md](../evaluation/experimental-study.md).
...
Relatório: `data/results/audit_dataset_leakage/leakage_report.{json,md}`.
...
python scripts/benchmark/run_benchmark.py --full \
  ...
  --out data/results/xgen_fkvoice_20260626 --verbose
```
...
python scripts/benchmark/run_benchmark.py --full \
  --dataset data/datasets/benchmark_audio_raw_balanced_15k.npz \
  --group-split \
  --out data/results/group_split_20260626 --verbose
```
...
- **P3.1** Reescrever o parágrafo de resultados: números in-domain rotulados como
  *teto otimista* + tabela cross-generator como estimativa honesta + ressalva do
  ...
  Arquivos: `data/results/paper/main.tex`, `data/results/paper/tabelas_benchmark.tex`.
- **P3.2** Documentar o protocolo em [../evaluation/benchmark.md](../evaluation/benchmark.md) e
  [../evaluation/experimental-study.md](../evaluation/experimental-study.md) (estratificado vs grupo
  vs cross-generator) + registrar esta auditoria.
- **P3.3** Consolidar figuras/tabelas a partir da corrida honesta:
  `python scripts/reporting/consolidate_results.py --input data/results/xgen_fkvoice_20260626 …`.
