...
Veja [Arquiteturas Neurais](../models/architectures.md) e a
[Revisão das Arquiteturas](../models/literature-review.md).
...
em [Features de Áudio](../architecture/audio-features.md).
...
- **Treino** pelo `TrainingService` (mesmo caminho da API e do benchmark), com
  ...
  modelo de inferência sem estado do otimizador. Ver [Treinamento](../models/training.md).
- **Inferência** pelo `ModelLoader` + `Predictor` (ONNX→TF com fallback),
  aplicando o `input_contract` (temperatura/EER/OOD). Ver
  [Inferência](../models/inference.md).
...
- **API FastAPI** (`/api/v1`): detecção single/multi-model, incerteza
  ...
  [API REST e Comunicação](../interfaces/rest-api.md).
...
- **Deploy/Qualidade**: Docker multi-stage, pytest, ruff/black, bandit,
  GitHub Actions. Ver [CI/CD e Segurança](../development/ci-cd-and-security.md).

## Próximos passos

Comece pela [Instalação e Configuração](installation.md). Termos
